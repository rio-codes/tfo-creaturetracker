...
export function CollectionClient({initialCreatures, totalPages}) {
    ...
    const handleFilterChange = useDebouncedCallback((filterName: string, value: string | boolean) => {
            ...
            <div className="container mx-auto px-4 py-5">
                ...
                <div className="flex flex-col lg:flex-row gap-4 mb-8">
                    ...
                    <div className="flex items-center gap-4">
                        <div className="flex items-center space-x-2">
                            <Checkbox 
                                id="female" 
                                defaultChecked={searchParams.get('female') !== 'false'}
                                onCheckedChange={(checked) => handleFilterChange('female', !!checked)}
                            />
                        ...
                        <div className="flex items-center space-x-2">
                            <Checkbox 
                                id="male" 
                                defaultChecked={searchParams.get('male') !== 'false'}
                                onCheckedChange={(checked) => handleFilterChange('male', !!checked)}
                            />
                        ...
                        <SelectContent className='bg-barely-lilac'>
                            <SelectItem value="all" className='bg-barely-lilac'>All Species</SelectItem>
                                {speciesList.map((species) => {
                                    return (
                                        <SelectItem key={species} value={species} className='bg-barely-lilac'>
                                        {species}
                                        </SelectItem>
                                    );
                                })}
                ...
                {initialCreatures.length > 0 ? (
                    ...
                    {initialCreatures.map((creature) => (
                        <CreatureCard creature={creature} allCreaturesData={initialCreatures} />
                    ))}
                </div>
                ) : (
