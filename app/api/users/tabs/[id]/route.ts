...
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
    ...
    try {
        ...
        const dataToUpdate: { tabName?: string; isSyncEnabled?: boolean; updatedAt: Date } = { updatedAt: new Date() };
        if (tabName !== undefined) dataToUpdate.tabName = tabName;
        if (isSyncEnabled !== undefined) dataToUpdate.isSyncEnabled = isSyncEnabled;

        const updatedTab = await db.update(userTabs).set(dataToUpdate).where(
            and(
                eq(userTabs.id, tabIdToUpdate),
                eq(userTabs.userId, userId)
            )
        ).returning();

        if (updatedTab.length === 0) {
            return NextResponse.json({ error: "Tab not found or not owned by user." }, { status: 404 });
        }
...
export async function DELETE(req: Request, { params }: { params: { id: string } }) {
    ...
    try {
        await db.delete(userTabs).where(
