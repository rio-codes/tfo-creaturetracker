        ...
        for (const [category, selection] of Object.entries(goal.genes)) {
            ...
            if (selection) {
                if (typeof selection === "object" &&
                    "phenotype" in selection && typeof selection.phenotype === "string" && selection.phenotype?.length > 0 &&
                    "genotype" in selection && typeof selection.genotype === "string" && selection.genotype?.length > 0 &&
                    selection.genotype) {
                    ...
                    const categoryData = speciesGeneData[category] as FullGeneSet;
